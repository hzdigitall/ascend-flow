-- Fix credit_points: Add cast to tx_direction
...
-- Fix credit_wallet: Add cast to tx_direction
...
BEGIN
  ...
  VALUES (_user, _wallet, 'in'::tx_direction, _cat, _amount, before_v, after_v, _desc, _ref);
END;
$$;

-- Fix request_withdrawal: Add cast to tx_direction
...
BEGIN
  ...
  VALUES (_user, _wallet, 'out'::tx_direction, 'withdrawal', _amount, bal, after_v, 'Solicitação de saque via PIX', wid, 'pending');
...
-- Security Hardening: Revoke public execute and grant only to service_role and authenticated
...
REVOKE EXECUTE ON FUNCTION public.redeem_product(uuid, uuid, jsonb) FROM public;
...
GRANT EXECUTE ON FUNCTION public.redeem_product(uuid, uuid, jsonb) TO authenticated, service_role;
