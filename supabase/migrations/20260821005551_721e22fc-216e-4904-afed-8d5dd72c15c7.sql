-- Atualizar a função confirm_payment para conceder pontos ao patrocinador direto
CREATE OR REPLACE FUNCTION public.confirm_payment(_payment UUID, _payload JSONB)
RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE pay RECORD; pl RECORD; sponsor UUID; lvl INT; pct NUMERIC; amt NUMERIC; rates JSONB; cur UUID; pts BIGINT;
BEGIN
    ...
    IF pct > 0 THEN
      ...
      IF amt > 0 THEN
        ...
        IF FOUND THEN
          ...
          -- NOVO: Conceder pontos ao patrocinador direto (Nível 1) no momento do depósito
          IF lvl = 1 AND pay.amount >= 50 THEN
             -- R$ 50 = 5 pontos (mesma lógica do plano)
